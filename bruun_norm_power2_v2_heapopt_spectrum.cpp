...
public:
          ...
          IDX(n / 2), OUTIDX(n / 2), C(n / 2), S(n / 2)
    {
        ...
        for (int m = 1; m < N / 2; ++m) {
            const int k = bruun_idx_int(m, L);
            IDX[m] = k;
            C[m] = 0.5 * T[k];           // cos(alpha)
            S[m] = 0.5 * T[N / 2 - k];   // sin(alpha)
        }
        ...
        while (true) {
            ...
            for (int o = 0; o < 2; ++o) {
                states[o].max_jump = 0;
                states[o].sum_jump = 0;
                const int a = pairs[0].first;
                const int b = pairs[0].second;
                if (o == 0) states[o].seq = {a, b};
                else        states[o].seq = {b, a};
            }

            for (size_t pi = 1; pi < pairs.size(); ++pi) {
                State next[2];
                for (int o = 0; o < 2; ++o) {
                    const int a = pairs[pi].first;
                    const int b = pairs[pi].second;
                    const int first = (o == 0) ? a : b;
                    const int second = (o == 0) ? b : a;

                    bool have = false;
                    State best;
                    for (int po = 0; po < 2; ++po) {
                        State cand = states[po];
                        const int j = cyclic_dist(cand.seq.back(), first);
                        cand.max_jump = std::max(cand.max_jump, j);
                        cand.sum_jump += j;
                        cand.seq.push_back(first);
                        cand.seq.push_back(second);

                        if (!have ||
                            cand.max_jump < best.max_jump ||
                            (cand.max_jump == best.max_jump && cand.sum_jump < best.sum_jump)) {
                            best = std::move(cand);
                            have = true;
                        }
                    }
                    next[o] = std::move(best);
                }
                states[0] = std::move(next[0]);
                states[1] = std::move(next[1]);
            }

            const int choose =
                (states[0].max_jump < states[1].max_jump ||
                 (states[0].max_jump == states[1].max_jump && states[0].sum_jump <= states[1].sum_jump)) ? 0 : 1;

            prev = states[choose].seq;
    ...
    // Standard FFTW-like real -> complex interface.
...
static void bench_one(int N, int forced_iters, FFTW* fftw, std::mt19937_64& rng) {
    ...
    for (int tr = 0; tr < trials; ++tr) {
        ...
        plan.forward(input.data(), X.data(), work.data());
...
int main(int argc, char** argv) {
    ...
    if (forced_N > 0) {
        ...
        const int sizes[] = {512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144};
        for (int N : sizes) {
